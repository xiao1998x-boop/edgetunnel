...
<body>
  <main class="shell">
    <section class="panel hero">
      <div class="hero-top">
        <div class="hero-copy">
          <span class="eyebrow">edgetunnel / admin shell</span>
          <h1>极简版 VPN 控制台</h1>
          <p>这一版只保留真正有用的管理区域: 节点、流量、日志、配置和 AI 生成链路。页面里的静态指标都是默认占位，后续接入真实 API 时不需要再推翻结构。</p>
        </div>
        <div class="hero-actions">
          <button id="refreshDebug" type="button" class="secondary">刷新诊断</button>
          <a class="button-link" href="/admin/debug" target="_blank" rel="noreferrer">查看调试 JSON</a>
        </div>
      </div>
      <div class="summary-grid">
        <div class="summary-item">
          <strong>KV 绑定</strong>
          <span id="summaryBinding">${绑定名}</span>
          <small>默认优先读取 env.bpb</small>
        </div>
        <div class="summary-item">
          <strong>ADMIN_UI</strong>
          <span id="summaryAdmin">${ADMIN_UI状态}</span>
          <small>未写入时显示当前后备后台</small>
        </div>
        <div class="summary-item">
          <strong>AI 模型</strong>
          <span id="summaryModel">${AI模型}</span>
          <small>默认固定为 GPT-5.4</small>
        </div>
        <div class="summary-item">
          <strong>密钥来源</strong>
          <span id="summarySource">${转义HTML(AI密钥来源)}</span>
          <small>优先环境变量，其次页面临时填写</small>
        </div>
      </div>
      ...
      <div class="column">
        <section class="panel">
          <div class="section-head">
            <div>
              <h2>节点列表</h2>
              <p>默认占位数据已经按常见 VPN 控制台布局排好，后续只需要接真实节点接口替换表体。</p>
            </div>
            <span class="section-tag">Host ${host}</span>
          </div>
          <table>
            <thead>
              <tr>
            ...
            <tbody id="nodeTableBody">
              <tr>
                <td><strong>HK-Gateway-01</strong><div class="node-meta">主入口 / ${host}</div></td>
              ...
              <tr>
                <td><strong>JP-Core-02</strong><div class="node-meta">常驻中转 / ${host}</div></td>
                <td><span class="node-pill online">在线</span></td>
                <td>Tokyo</td>
                <td>52 ms</td>
                <td>58%</td>
              ...
              <tr>
                <td><strong>US-West-Edge</strong><div class="node-meta">备援出口 / ${host}</div></td>
              ...
              <tr>
                <td><strong>SG-Failover</strong><div class="node-meta">自动接管 / ${host}</div></td>
        ...
        <section class="panel">
          <div class="section-head">
            <div>
              <h2>流量概览</h2>
              <p>把原先冗长的图形卡片压缩成一组简洁摘要，更适合先做后备后台。</p>
            </div>
            <span class="section-tag">24h 占位数据</span>
          </div>
          <div class="metric-grid">
            <article class="metric-card">
              <strong>24h 总流量</strong>
              <div class="metric-value">842 GB</div>
              <div class="metric-note">下载 522 GB / 上传 320 GB</div>
            </article>
            <article class="metric-card">
              <strong>活跃会话</strong>
              <div class="metric-value">2,184</div>
              <div class="metric-note">默认样例，可替换为在线连接数</div>
            </article>
            <article class="metric-card">
              <strong>平均延迟</strong>
              <div class="metric-value">48 ms</div>
              <div class="metric-note">香港入口当前表现最佳</div>
            </article>
          </div>
          <div class="bar-list" id="trafficChart">
            <div class="bar-row"><strong>香港</strong><div class="bar-track"><span style="width:82%"></span></div><div>312 GB</div></div>
            <div class="bar-row"><strong>东京</strong><div class="bar-track"><span style="width:69%"></span></div><div>264 GB</div></div>
            <div class="bar-row"><strong>新加坡</strong><div class="bar-track"><span style="width:44%"></span></div><div>168 GB</div></div>
            <div class="bar-row"><strong>北美</strong><div class="bar-track"><span style="width:26%"></span></div><div>98 GB</div></div>
          </div>
        </section>
        ...
        <section class="panel">
          <div class="section-head">
            <div>
              <h2>系统日志</h2>
              <p>只保留初始化、诊断、保存和 AI 生成这几类最关键的事件。</p>
            </div>
            <span class="section-tag">事件流</span>
          </div>
          <div class="log-list" id="eventLog">
            <div class="log-item">
              <div class="log-meta"><span>BOOT</span><span>刚刚</span></div>
              <div>默认管理后台已经切换为极简版壳层，当前绑定 <code>${绑定名}</code>。</div>
            </div>
            <div class="log-item">
              <div class="log-meta"><span>KV</span><span>实时</span></div>
              <div>当前 <code>ADMIN_UI</code> 状态为 <strong>${ADMIN_UI状态}</strong>，未写入时会显示这套默认壳。</div>
            </div>
            <div class="log-item">
              <div class="log-meta"><span>AI</span><span>待命</span></div>
              <div>AI 中转地址 <code>${AI地址}</code>，模型 <code>${AI模型}</code>，密钥来源为 <strong>${转义HTML(AI密钥来源)}</strong>。</div>
            </div>
          </div>
        </section>
      </div>
      ...
      <aside class="column">
        <section class="panel">
          <div class="section-head">
            <div>
              <h2>基础配置</h2>
              <p>保存最常改的四个字段，避免每次生成后台都重复输入。</p>
            </div>
            <span class="section-tag">本地存储</span>
          </div>
          <div class="field-grid">
            ...
            <div>
              <label for="cfgPath">路径</label>
              <input id="cfgPath" type="text" value="/" />
            </div>
            <div>
              <label for="cfgMode">传输模式</label>
          ...
          <div class="hint">这里只做浏览器本地保存，后续如果要接真实配置接口，再让 AI 在当前结构上继续扩展即可。</div>
        </section>
        ...
        <section class="panel">
          <div class="section-head">
            <div>
              <h2>AI 工作台</h2>
              <p>继续把复杂后台需求交给 ${AI模型} 生成，结果会直接写入 <code>ADMIN_UI</code>。</p>
            </div>
            <span class="section-tag">/api/codex-init</span>
          </div>
          ...
          <div class="hint">建议把 <code>CPA_API_BASE</code>、<code>CPA_API_KEY</code>、<code>ADMIN_UI_MODEL</code> 固定在 Cloudflare Pages 环境变量里，页面填写只作为当前请求覆盖。</div>
          <div class="actions">
