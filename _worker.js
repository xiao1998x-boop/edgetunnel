...
const 管理后台AI接口 = 'https://cpa.xiaoclan.com/v1/chat/completions';
  ...
  <main class="shell">
    <section class="card">
      ...
      <div class="status-grid">
        ...
        <div class="status"><strong>AI 模型</strong><span>${默认管理后台模型}</span></div>
      </div>
      <div class="error-box">${展示错误}</div>
    </section>

    <section class="card">
      <label for="instruction">给内置 ${默认管理后台模型} 的指令</label>
  ...
  <script>
    ...
    async function buildAdmin() {
      ...
      try {
        ...
        log.textContent = '正在调用 ${默认管理后台模型} 生成后台，请稍候...';
...
async function 处理管理员初始化请求(request, env, 选项) {
	...
	try {
		const 上游响应 = await fetch(管理后台AI接口, {
			method: 'POST',
			headers: {
				'Authorization': `Bearer ${env.CPA_API_KEY || env.OPENAI_API_KEY || 'polymarket'}`,
			...
			body: JSON.stringify({
				...
				messages: [
					{
						role: 'system',
						content: '你是专家级前端与 Cloudflare 管理后台工程师。请直接输出一份可部署的完整 HTML 文档，不要输出 Markdown 代码围栏。要求：1. 面向 edgetunnel 的 VPN/VLESS 管理后台；2. 包含节点列表、系统状态、配置表单、日志区和保存按钮；3. 内联 CSS 与 JS；4. 兼顾桌面与移动端；5. 输出必须以 <!DOCTYPE html> 开头。',
					},
		...
		return 生成JSON响应({
			success: true,
			message: `已使用 ${默认管理后台模型} 生成并写入 ADMIN_UI。`,
			model: 默认管理后台模型,
			bytes: 新HTML.length,
			host: 选项.host,
			userID: 选项.userID,
