...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/admin') {
      // 从 KV 读取 UI
      let html = await env.bpb.get('ADMIN_UI');
      
      // 如果 KV 是空的，展示一个极简的“AI 引导安装程序”
      if (!html) {
        html = `
          <!DOCTYPE html>
          <html>
          <head><title>Codex 引导程序</title></head>
          <body style="background:#000;color:#0f0;padding:20px;font-family:monospace;">
            <h3>[系统检测]：ADMIN_UI 为空</h3>
            <p>正在连接 GPT-5.4 构建初始界面...</p>
            <textarea id="cmd" style="width:100%;height:100px;background:#111;color:#0f0;" placeholder="描述你想要的面板样子，例如：给我一个暗黑风格、带节点列表和流量监控的 VPN 管理后台"></textarea>
            <button onclick="build()" style="margin-top:10px;padding:10px;cursor:pointer;">唤醒 Codex 生成界面</button>
            <div id="log"></div>

            <script>
              async function build() {
                const log = document.getElementById('log');
                log.innerText = '正在通过 GPT-5.4 编写源码并注入 KV...请稍候...';
                ...
                alert(result);
                location.reload(); // 成功后刷新，就会看到 AI 写好的新界面
              }
            </script>
          </body>
          </html>
    ...
    // 2. AI 初始构建接口 (内部调用)
    if (url.pathname === '/api/codex-init' && request.method === 'POST') {
      const userReq = await request.text();
      
      // 这里的配置是根据你之前的 API 中转设置的
      const aiRes = await fetch("https://cpa.xiaoclan.com/v1/chat/completions", {
        method: "POST",
        headers: { 
          "Authorization": "Bearer polymarket", 
          "Content-Type": "application/json" 
        },
        body: JSON.stringify({
          model: "gpt-5.4", // 强制使用 GPT-5.4 确保代码质量
          messages: [
            { role: "system", content: "你是一个专家级的 VPN 面板开发者。请直接输出一套包含完整 CSS 和 JS 的 HTML 代码，用于管理 VLESS 节点。界面要高端、响应式。" },
      ...
      // 将 AI 生成的代码直接存入 KV
      await env.bpb.put('ADMIN_UI', newHtml);
      return new Response("第一版 UI 注入成功！");
    }

    // ...这里保留你原本处理 edgetunnel 流量转发的其他逻辑...
    //（建议把 edgetunnel 原本的逻辑粘在下面）
  }
};
///////////////////////////////////////////////////////////////////////XHTTP传输数据///////////////////////////////////////////////
